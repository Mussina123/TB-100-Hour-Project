...
const DiaryDetails = ({diary}) => {
    ...
    return (
        <div className="flex flex-col m-auto md:ml-6 mt-6 shadow-lg shadow-slate-500/20 rounded-xl relative w-60" >
            <h1 className="text-3xl font-semibold mb-2" key={diary._id}> {diary.item}</h1>
            <h3 className="italic mb-1"> Calories: {diary.calories}</h3>
            <h3 className="text-slate-600 mb-1"> Serving: {diary.serving}</h3>
